                ...
                let env =
                  ...
                  |> option_value
                       ~message:
                         ...
                         @@ Runtime.to_s env)
                in
                match M.proc_exec ~mode:ForkBuffer self env with
                | Ok t' -> First t'
                ...
                let env =
                  ...
                  |> option_value
                       ~message:
                         ...
                         @@ Runtime.to_s env)
                in
                match M.proc_exec ~mode:ForkInherit self env with
                | Ok t' -> First t'
                | Error err -> Second (Runtime.Error (None, String err))) );
          ( "blockBuffer",
                ...
                let env =
                  ...
                  |> option_value
                       ~message:
                         ...
                         @@ Runtime.to_s env)
                in
                match M.proc_exec ~mode:BlockBuffer self env with
                | Ok t' -> First t'
                | Error err -> Second (Runtime.Error (None, String err))) );
          ( "blockInherit",
                ...
                let env =
                  ...
                  |> option_value
                       ~message:
                         ...
                         @@ Runtime.to_s env)
                in
                match M.proc_exec ~mode:BlockInherit self env with
                | Ok t' -> First t'
                | Error err -> Second (Runtime.Error (None, String err))) );
          ( "stdout",
            fun self ->
